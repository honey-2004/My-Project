...
namespace Iovista\Customization\Controller\Account;
...
class CreatePost extends \Magento\Customer\Controller\Account\CreatePost implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    protected function validateNames($name)
    {
        $nameLeng = strlen($name);

        if(($nameLeng >= 2 && $nameLeng <= 12) && // To check length
            (preg_match('/[^a-zA-Z0-9]/', $name) == 0) && // To check special characters
            (!preg_match('~[0-9]+~', $name)) // To check numbers
        ) {
            return false;
        }
    ...
    public function execute()
    {
        ...
        if(isset($customerData['firstname']) && $customerData['firstname'] != NULL){
            $firstname = $customerData['firstname'];
            $validateFirstName = $this->validateNames($firstname);

            if($validateFirstName){
                $this->messageManager->addErrorMessage(__('First Name length must be between 2 to 12 and not contain special characters or numbers.'));
                return $resultRedirect->setUrl($this->_redirect->error($defaultUrl));
            }
        }

        if(isset($customerData['lastname']) && $customerData['lastname'] != NULL){
            $lastname = $customerData['lastname'];
            $validateFirstName = $this->validateNames($lastname);

            if($validateFirstName){
                $this->messageManager->addErrorMessage(__('Last Name length must be between 2 to 12 and not contain special characters or numbers.'));
                return $resultRedirect->setUrl($this->_redirect->error($defaultUrl));
            }
        }
